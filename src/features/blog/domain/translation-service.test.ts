...
describe('TranslationService', () => {
  it('should translate a given key', () => {
    ...
    const actual = translationService.translate(Locale.EN, 'existingKey' as keyof TranslationIdentifiers)
  ...
  it('should give default translation of a given key if it does not exist in the given locale', () => {
    const { translationService } = setup()
    const actual = translationService.translate(Locale.ES, 'nonExistingKeyInSpanish' as keyof TranslationIdentifiers)
  ...
  it('should error if translation is not found neither in the given locale or the default locale', () => {
    ...
    }).toThrowError(new TranslationError('Translation for key "nonExistingKey" in locale "1" could not be found'))
  })

  it('should return the default locale if the given locale does not exist', () => {
    const { translationService } = setup()
    const NON_EXISTING_LOCALE = 99
    const actual = translationService.translate(NON_EXISTING_LOCALE, 'existingKey' as keyof TranslationIdentifiers)
  ...
  it('should throw an error if string is not matched to locale', () => {
    ...
    }).toThrowError(new TranslationError('String NON_EXISTING_LOCALE could not be mapped to a locale'))
  })
})
