...
export class Links extends LitElement {
  ...
  render() {
    return html`<div class="${classMap({ [this.links]: true, links: true })}">
      <a @click="${() => this.onLinkClick()}" href="/" class="link home">${subscribe(this.translations.home)}</a>
      <a @click="${() => this.onLinkClick()}" href="/talks" class="link">${subscribe(this.translations.talks)}</a>
      <a @click="${() => this.onLinkClick()}" href="/about" class="link">${subscribe(this.translations.about)}</a>
    </div>`
  }
}
