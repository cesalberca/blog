...
A powerful way of conveying your message –whatever that might be– is to make sure the **medium in which your message is transmitted is attractive enough**. Designing is something that we humans do quite often, we design our outfits, we design how we talk and me as web designer, I designed this very same site you are experiencing.

## Technologies

Technology _needs_ to serve a purpose. Sometimes –most times, let's be honest– us web developers we focus too much in the technologies and frameworks we use. We should, but only to a _certain_ extent. I'm mentioning some of the technologies I've used, what I've learned from then and how they empowered me to focus on what matters; the message.
...
A good starting point is to use starting points. I used [Next.js starter porfolio](https://vercel.com/templates/next.js/portfolio-starter-kit). It provided me with some configuration, specifically about SEO so I wouldn't worry initially about it. For instance it provided `sitemap.ts`, `robots.ts`, `rss/route.ts` and `og/route.ts.`. What I usually do is I generate in a temporal folder the template and then I choose what to copy over to the project. This ensures that I don't take more than I need to and that I can check exactly what I want to bring over.
...
I made a conscious –perhaps also debatable?– choice of not to delve too deep in **best practices**, **testing** and **architecture**. _Why_? Because I want to create a blog series where I can improve the codebase of code bit by byte. But hey, it's not like I coded it with my eyes closed (which could be an interesting challenge, to be honest). You can check the source code here at the time of publishing of this post.

However, I did decide to focus on creating a clean design with a big focus on content and some subtle animations that would make the site b·e·a·u·t·i·f·u·l.

<div className="grid grid-columns-3 gap-m">
  <Background>
    <div className="p-xl">Background</div>
  </Background>
  <div><NavLink to="#">Navbar link</NavLink></div>
  <div><AccentText repeat>Accent text</AccentText></div>
  <div><AccentList texts={['One', 'Two', 'Forty two']}></AccentList></div>
  <AccentCard className="flex items-center justify-center">
    <div className="p-xl">You can't really print this interaction</div>
  </AccentCard>
</div>
...
<Background><div className="p-xl">Bar</div></Background>
